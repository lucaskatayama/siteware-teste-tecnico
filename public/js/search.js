$(document).ready(function () {
    var url_api = '//api.openweathermap.org/data/2.5/find?q={query}' +
        '&type=like' +
        '&units=metric' +
    ...
    $('#search-for-weather')
        .search({
            ...
            onSelect: function (result,response) {
                getWeatherInformation(result, "find");
            }
        })
    ;

    $('#favorite-segment').on('click', '.eye-favorite-icon-action-btn', function() {
        var city_id = $(this).find(".city-id").text();

        var url_api_find = '//api.openweathermap.org/data/2.5/weather?id=' + city_id +
            '&units=metric' +
            '&appid=232c8c2ef0ee39175db1823b9fe7cc1b';
        ...
        $.ajax({
            ...
            dataType: "json",
            success: result => {
                getWeatherInformation(result, "favorite");
            },
        });
    });
});
...
function getWeatherInformation(result, parameter) {
    if (parameter === "find")
